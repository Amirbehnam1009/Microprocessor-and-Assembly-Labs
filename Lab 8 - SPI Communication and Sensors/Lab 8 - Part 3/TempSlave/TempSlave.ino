/*
 * SPI Temperature Sensor Slave Receiver
 * Receives temperature values from SPI master with interrupt-driven protocol
 */
...
volatile uint8_t temperatureValue = 0;    // Stores received temperature value in Celsius
...
void setup() {
  ...
  Serial.println("Starting Temperature Sensor Slave"); // Startup message
 ...
 * Processes received temperature data from SPI master
 */
void loop() {
  if (dataReceived) {
    processTemperatureData();            // Handle the received data
    resetReceiverState();                // Prepare for next data reception
  }
}

/**
 * Processes received temperature data
 * Outputs the value to serial monitor with proper formatting
 */
